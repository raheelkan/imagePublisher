...
int main(int argc, char **argv)
{
  ...
  //  }
    ...
    int nImages = 300;//vstrImageFilenames.size();
    ...
    cerr << endl << "Publishing Frames to the topic /image_raw (For PTAM)...from: " << argv[1]<< endl;
    int ni = 0;
	...
	while (nh.ok() && ni<imageNames.size()) {
	  //cap >> frame;
	    ...
	    if(!frame.empty()) {
	        ...
	        msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", frame).toImageMsg();//mono8//bgr8
